...
class MeetingsCog(commands.Cog):
    ...
    async def create_meeting(
        ...
        embed = discord.Embed(title="✅ Réunion créée", color=discord.Color.green())
    ...
    @is_admin()
    async def start_attendance(self, interaction: discord.Interaction, meeting_id: int):
    ...
    async def list_meetings(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="📅 Prochaines réunions", color=discord.Color.blue()
        )
...
class AttendanceView(discord.ui.View):
    ...
    async def present(
        ...
        if interaction.user.id in self.attendees:
            await interaction.response.send_message(
                "✅ Déjà marqué présent", ephemeral=True
            )
            return
        ...
        await interaction.response.send_message(
            "✅ Présence enregistrée", ephemeral=True
        )
