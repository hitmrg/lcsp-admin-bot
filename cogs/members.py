...
class MembersCog(commands.Cog):
    ...
    async def add_member(
        ...
        embed = discord.Embed(title="✅ Membre ajouté", color=discord.Color.green())
    ...
    # Permet de consulter les informations d'un membre
    ...
    async def member_info(
        ...
        embed.add_field(
            name="Membre depuis", value=member.joined_at.strftime("%d/%m/%Y")
        )
    ...
    async def update_member(
        ...
        if member:
            await interaction.followup.send(
                f"✅ {user.mention} mis à jour", ephemeral=True
            )
        else:
    ...
    async def delete_member(
        ...
        if self.db.delete_member(str(user.id)):
            await interaction.followup.send(
                f"✅ {user.mention} supprimé", ephemeral=True
            )
        else:
            await interaction.followup.send(f"❌ Membre non trouvé", ephemeral=True)
    ...
    async def list_members(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title=f"👥 Membres ({len(members)})", color=discord.Color.blue()
        )
